...
export function ProductSlider() {
    ...
    const { setAvailableColorsInPopup, setCurrentColorInPopup, currentColorInProducts } = useFilterStore();
    const openPopup = useCallback((product: IProduct, color: Colors) => {
        setAvailableColorsInPopup(Object.keys(product.imagesByColor) as Colors[]);
        setCurrentColorInPopup(color);
        open(`${product.name}`, <ProductInfo product={product} />);
    }, [open]);

    return (
        <SwiperCustom title="View our products">
            {products.map((product: IProduct) => {
                return Object.entries(product.imagesByColor)
                    .filter(([color]) => !currentColorInProducts || color === currentColorInProducts)
                    .map(([color, images]) => {
